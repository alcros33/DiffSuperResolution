...
class UNetConditionalCat(nn.Module):
    def __init__(
        ...
        for level in range(num_resolutions):
            ...
            for _ in range(num_res_blocks):
                ...
                if apply_attention[level]:
                    if max_vit_attn:
                        layers.append(MaxViTAttention(in_channels, n_heads, window_size=window_size, dropout=dropout_rate))
                    else:
        ...
        self.bottleneck_blocks = nn.ModuleList([ResBlockWithTime(in_channels, in_channels,
                                            ...
                                            #     MaxViTAttention(in_channels, n_heads, window_size=window_size, dropout=dropout_rate) if max_vit_attn else
            ...
            for it in range(num_res_blocks+1):
                ...
                if apply_attention[level]:
                    if max_vit_attn:
                        ...
                        layers.append(AttentionBlock(in_channels, n_heads, dropout=dropout_rate, window_size=window_size))
...
class UnetConditionalCompact(nn.Module):
    def __init__(
        self,
        time_steps,
        input_channels=3,
        ...
        for level in range(num_resolutions):
            out_channels = base_channels * base_channels_multiples[level]
            curr_channels.append(out_channels)
            layers = []
            if level !=0:
            ...
            if apply_attention[level]:
                if max_vit_attn:
                    ...
                    layers.append(AttentionBlock(in_channels, n_heads, dropout=dropout_rate))
            # TODO MAYBE ADD 1 BLOCK AFTER ATTENTION
            self.encoder_blocks.append(TimestepEmbedSequential(*layers))
